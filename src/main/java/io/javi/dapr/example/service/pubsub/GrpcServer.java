...
import io.javi.dapr.example.interceptors.MetadataInterceptor;
...
public class GrpcServer extends AppCallbackHealthCheckImplBase {
  ...
  public void start() {
    new Thread(() -> {
      ...
      serverBuilder = MetadataInterceptor.addMetadataIntercepor(serverBuilder);

      if (consumerEnabled) {
        logger.info("Enabling Consumer");
        serverBuilder.
